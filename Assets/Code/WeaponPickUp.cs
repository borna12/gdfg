...
public class WeaponPickUp : MonoBehaviour
{
	...
	void Awake(){
		...
		ammo=PlayerPrefs.GetInt("ammo count1");

		if(profile.profile2==true)
			ammo=PlayerPrefs.GetInt("ammo count2");

		if(profile.profile3==true)
			ammo=PlayerPrefs.GetInt("ammo count3");
		...
		if (collided == true) {
				...
				AmmoCountGUI.GetComponent<TextMesh> ().text ="";
						...
						if (Input.GetKeyDown (KeyCode.Alpha3)) {
								if (player.GetComponent<Player> ().CurrentWeapon == gameObject)
										gameObject.SetActive (false);
								player.GetComponent<Player> ().CurrentWeapon = player.GetComponent<Player> ().Weapon3;
				AmmoCountGUI.GetComponent<TextMesh> ().text ="";
			}
			if (player.GetComponent<Player> ().CurrentWeapon == gameObject) {
